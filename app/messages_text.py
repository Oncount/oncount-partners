"""Тексты сообщений бота — вынесены отдельно. Формат: Telegram HTML.

В HTML экранировать нужно только &, <, >. Подчёркивания и URL — без проблем.
"""

WELCOME_NEW = (
    "Привет, {first_name}! 👋\n\n"
    "Я — бот On Count: лицензированной бухгалтерии полного цикла в ОАЭ.\n\n"
    "С чего хочешь начать?\n\n"
    "📅 <b>Мастер-класс «AI 2-й мозг»</b> — 21 мая, 18:00 (Дубай). Бесплатно.\n"
    "🤝 <b>Партнёрская программа</b> — приводи клиентов, получай комиссию.\n\n"
    "Выбери внизу 👇"
)

WELCOME_REGISTERED_FOR_EVENT = (
    "Привет, {first_name}! 👋\n\n"
    "Ты уже зарегистрирован/-а на мастер-класс <b>«AI 2-й мозг»</b> 21 мая в 18:00 (Дубай).\n\n"
    "🎥 Zoom: https://us06web.zoom.us/j/81890945980\n\n"
    "Хочешь параллельно стать партнёром OnCount? Жми кнопку ниже."
)

WELCOME_PARTNER = (
    "С возвращением, {first_name}! 🤝\n\n"
    "Что хочешь сделать?"
)

EVENT_REGISTERED = (
    "Готово, {first_name}! 🎉\n\n"
    "Ты зарегистрирован/-а на бесплатный AI-мастер-класс <b>«AI 2-й мозг»</b> от On Count.\n\n"
    "📅 <b>21 мая 2026, среда, 18:00 (Дубай / GST)</b>\n"
    "🎥 Онлайн в Zoom — ссылку пришлю в день мероприятия.\n\n"
    "---\n\n"
    "<b>Подготовка к мастер-классу — 6 этапов. Делай по порядку:</b>\n\n"
    "🖥 Этап 1. Устанавливаем VS Code — ~5 мин\n"
    "🔐 Этап 2. Регистрируемся в Claude — ~20–30 мин\n"
    "💳 Этап 3. Оплачиваем подписку — ~10 мин\n"
    "⚙️ Этап 4. Ставим Claude Code и настраиваем — ~15 мин\n"
    "🎤 Этап 5. Голосовой ввод + скриншоты — ~10 мин\n"
    "🤖 Этап 6. Запускаем агента — ~10–20 мин\n\n"
    "📄 <b>Пошаговая инструкция:</b>\n"
    "https://docs.google.com/document/d/18rpKrqfDK1dIBIz5Psf7W8aRF3aW2_vuW2KBG3FsTcs/edit?usp=sharing"
)

EVENT_REMINDER_24H = (
    "Напоминаю: <b>завтра, 21 мая, в 18:00</b> (Дубай) — мастер-класс «AI 2-й мозг».\n\n"
    "✅ Проверь по чек-листу, что всё установлено.\n"
    "✅ Подготовь блокнот и наушники.\n"
    "✅ Заложи 3 часа — будет много практики.\n\n"
    "До завтра!"
)

EVENT_REMINDER_ZOOM = (
    "Сегодня в <b>18:00</b> (Дубай) встречаемся на мастер-классе.\n\n"
    "🎥 <b>Ссылка на Zoom:</b> https://us06web.zoom.us/j/81890945980\n\n"
    "Сохрани, чтобы не искать. До вечера!"
)

EVENT_REMINDER_1H = (
    "Через час начинаем — <b>заходи</b> 🚀\n\n"
    "🎥 https://us06web.zoom.us/j/81890945980"
)

PARTNER_ONBOARDING_INTRO = (
    "🤝 <b>Партнёрская программа On Count</b>\n\n"
    "Зарабатывай вместе с лицензированной бухгалтерией №1 для русскоязычного бизнеса в ОАЭ.\n\n"
    "<b>Почему это выгодно:</b>\n"
    "• 🛡 <b>Надёжный партнёр.</b> Лицензия ОАЭ, страхование ответственности 1 000 000 AED, команда бухгалтеров с опытом 8+ лет.\n"
    "• 💰 <b>Щедрая комиссия — и вовремя.</b>\n"
    "   ▸ <b>100% от первого месяца</b> бухгалтерского обслуживания клиента;\n"
    "   ▸ <b>30%</b> от разовых услуг (аудит, налоговые отчёты, открытие компании, визы) — обычно $500–1 000 с клиента;\n"
    "   ▸ <b>10%</b> от агентских вознаграждений партнёров, которых ты привёл в сообщество;\n"
    "   ▸ <b>White Label — 30% ежемесячно</b> с каждой оплаты, если у тебя 10+ клиентов на бухгалтерии.\n"
    "   Выплаты — раз в месяц по итогам, акт + перевод на твои реквизиты.\n"
    "• 🔗 <b>Готовые инструменты.</b> Персональные реф-ссылки (Telegram, WhatsApp, сайт) и QR-коды — копируешь и отправляешь клиенту.\n"
    "• 📨 <b>Готовые тексты</b> рассылок под 5 сегментов (юристы / free-zone / банкиры / коворкинги / бухгалтеры) — не нужно ничего писать самому.\n"
    "• 📊 <b>Прозрачный дашборд.</b> Видишь каждого приведённого клиента, статус сделки в CRM, начисленную и выплаченную комиссию — всё в одном кабинете.\n"
    "• ⚡ <b>1 клик — передал клиента.</b> Прямо в этом боте: имя → контакт → задача. Менеджер свяжется в течение часа, остальное мы делаем сами.\n\n"
    "Открой свой кабинет — там уже ждут твои ссылки, тексты и статистика:"
)

PARTNER_LINKS = (
    "🔗 <b>Твои реферальные ссылки:</b>\n\n"
    "<b>Для приглашения в бот:</b>\n"
    "<code>{bot_link}</code>\n\n"
    "<b>Для размещения на сайте:</b>\n"
    "<code>{site_link}</code>\n\n"
    "Все ссылки + QR-код — в личном кабинете."
)

TRANSFER_INTRO = (
    "📍 <b>Передать клиента</b>\n\n"
    "Введи имя клиента:"
)

TRANSFER_DONE = (
    "✅ Клиент <b>{name}</b> передан в работу.\n\n"
    "Менеджер свяжется с ним в течение 24 часов. "
    "Статус сделки появится в твоём ЛК."
)
