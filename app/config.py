...
class Settings:
    ...
    # Телеграм-канал Wazzup («tgapi», рабочий аккаунт ONCOUNT +971 58 921 7784).
    # Нужен сторожу зависших заявок: боту Telegram запрещает писать первым тому,
    # кто не нажимал START, а аккаунту, с которым у человека есть переписка, нет.
    # Пусто → сторож не отправляет ничего и говорит об этом в отчёте.
    WAZZUP_TG_CHANNEL_ID: str = os.getenv("WAZZUP_TG_CHANNEL_ID", "")
    # Сторож зависших заявок (решение Николь 20.09.2026). По умолчанию OFF:
    # это автоматическая отправка живым людям, включается осознанно переменной.
    NUDGE_ENABLED: bool = os.getenv("NUDGE_ENABLED", "") in ("1", "true", "True")
    # Потолок на один прогон сторожа. Правило Николь: не больше 30 за раз.
    NUDGE_MAX_PER_RUN: int = int(os.getenv("NUDGE_MAX_PER_RUN", "30") or 30)
    # Бот AI-Стаси, продажи в переписках (@SalesAR_bot). Решение Николь
    # 21.09.2026: отчёты сторожа приходят в пульт Стаси, потому что письма
    # сторожа — это переписка, а переписки ведёт она. Бот пишет только Николь
    # (ADMIN_TG_ID). Пусто → отчёт уходит старым путём, через бот ONCOUNT,
    # чтобы не потеряться.
    STASYA_BOT_TOKEN: str = os.getenv("STASYA_BOT_TOKEN", "")
    # Ключ ТОЛЬКО на чтение очереди сторожа (`/admin/api/nudge-queue`). Отдельный
    # от CHANNEL_TAGS_TOKEN намеренно: тот открывает и живой запуск сторожа, а
    # серверу AI-Стаси нужна одна дверь — посмотреть, кому сторож напишет.
    # Утечёт — прочтут очередь, но не отправят ни одного письма.
    NUDGE_QUEUE_TOKEN: str = os.getenv("NUDGE_QUEUE_TOKEN", "")
