...
def build_digest(partner: Partner, session: Session) -> str | None:
    ...
    lines = [
        ...
        f"⏳ в работе: {len(opn)}",
        f"✅ оплат: {len(won)}",
        f"💰 Ожидаемая комиссия: ${total * EXP_LOW:,} – ${total * EXP_HIGH:,}",
    ...
    lines += [
        ...
        "",
        "Ждём ещё от Вас клиентов на бухгалтерский сервис в ОАЭ, заработаем вам комиссию.",
        "В личном кабинете свежие рефссылки и посты для рассылок.",
    ]
    return "\n".join(lines)
