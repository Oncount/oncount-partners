...
# Состояние кассы на 23.09.2026: все тарифы переведены в евро, два переименованы,
# добавлена «Консультация». Id при переименованиях не менялись — в этом и грабля.
OFFER_FIRST_DAY = "85dc8277-8980-4cb1-9fd1-ab4ed12215fb"   # «Интенсив AI-сотрудники», 20 €
OFFER_INTENSIVE = "2449b65d-b100-4d76-8d0b-c0dd4882e53e"   # «Курс AI-сотрудники, 10 онлайн встреч», 1 000 €
OFFER_DFY = "effe38ce-e866-46d0-89ab-5397e94f66ec"         # «Индивидуальная настройка», 2 500 €
OFFER_CONSULT = "d9958023-5955-4a73-8124-5deafe30ecc5"     # «Консультация», 350 €
...
PRODUCTS: dict[str, tuple[str, str]] = {
    ...
    #
    # ⚠️ Подпись короткая намеренно (04.08.2026, замечание Николь): к ней бот
    # дописывает цену, а Telegram на телефоне обрезает длинную надпись кнопки —
    # обрезается как раз хвост, то есть цена. Описание тарифа человек только что
    # прочитал на лендинге; на кнопке важно ЧТО и СКОЛЬКО.
    # Коды продуктов не трогаем: они уже лежат в intensive_leads.product_code.
    # Меняются только подписи — под то, как тарифы называются в кассе сейчас.
    "first_day": (OFFER_FIRST_DAY, "Интенсив"),
    "intensive": (OFFER_INTENSIVE, "Курс"),
}
...
def check_offers() -> list[str]:
    ...
    if not is_configured():
        return []
    # Маркеры — куски текущих названий в кассе, сверено 23.09.2026. Меняется
    # название в кассе — меняется маркер здесь, иначе бот при старте ругается.
    expect = {OFFER_FIRST_DAY: "интенсив", OFFER_INTENSIVE: "курс",
              OFFER_DFY: "настройк", OFFER_CONSULT: "консультац",
              OFFER_CLUB: "клуб"}
    problems: list[str] = []
    try:
        r = httpx.get(f"{BASE_URL}/api/v2/products", headers=_headers(),
